...
namespace SEeToDoList.Logic.Modules.CodeGenerator
{
    /// <summary>
        ...
        public static Type[] AllTypes
        {
            get
            {
                var allTypes = AppDomain.CurrentDomain
                                        .GetAssemblies()
                                        .Where(a => a.FullName!.StartsWith($"{nameof(SEeToDoList)}{Common.StaticLiterals.CommonExtension}")
                                                 || a.FullName!.StartsWith($"{nameof(SEeToDoList)}{Common.StaticLiterals.LogicExtension}"))
                                        .SelectMany(assembly =>
                                        {
                                            try
                                            {
